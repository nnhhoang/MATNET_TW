...
class MixedScore_MultiHeadAttention(nn.Module):
    def __init__(self, **model_params):
        ...
        mix1_weight = torch.torch.distributions.Uniform(low=-mix1_init, high=mix1_init).sample((head_num, 3, ms_hidden_dim)) ###fix here
    ...
    def forward(self, q, k, v, cost_mat, cost_time):
        ...
        time_mat_score = cost_time[:, None, :, :].expand(batch_size, head_num, row_cnt, col_cnt)
        # shape: (batch, head_num, row_cnt, col_cnt)
        two_scores = torch.stack((dot_product_score, cost_mat_score, time_mat_score), dim=4)  ##fix here
