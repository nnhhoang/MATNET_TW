...
class ATSPModel(nn.Module):
    ...
    def pre_forward(self, reset_state):
        ...
        self.encoded_row, self.encoded_col = self.encoder(row_emb, col_emb, cost, time)
...
class ATSP_Encoder(nn.Module):
    ...
    def forward(self, row_emb, col_emb, cost_mat,cost_time):
        # col_emb.shape: (batch, col_cnt, embedding)
        # row_emb.shape: (batch, row_cnt, embedding)
        # cost_mat.shape: (batch, row_cnt, col_cnt)

        for layer in self.layers:
            row_emb, col_emb = layer(row_emb, col_emb, cost_mat,cost_time)
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, row_emb, col_emb, cost_mat,cost_time):
        ...
        # col_emb.shape: (batch, col_cnt, embedding)
        # cost_mat.shape: (batch, row_cnt, col_cnt)
        row_emb_out = self.row_encoding_block(row_emb, col_emb, cost_mat,cost_time)
        col_emb_out = self.col_encoding_block(col_emb, row_emb, cost_mat.transpose(1, 2),cost_time.transpose(1, 2))
...
class EncodingBlock(nn.Module):
    ...
    def forward(self, row_emb, col_emb, cost_mat, cost_time):
        ...
        out_concat = self.mixed_score_MHA(q, k, v, cost_mat, cost_time)
